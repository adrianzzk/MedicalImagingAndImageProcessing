...
def split_lab(img,outdir,name,map_dic):
    ...
    for i in range(img_arr.shape[0]):
        ...
        slice_arr=img_arr[i]
        tmp_name=f"{outdir}/{name}_{i:04}.png"
        print(tmp_name)
        save_lab_to_png(slice_arr, tmp_name,map_dic)
...
def split_img(img,outdir,name):
    ...
    for i in range(img_arr.shape[0]):
        slice_arr=img_arr[i]
        tmp_name=f"{outdir}/{name}_{i:04}.png"
        save_img_to_png(slice_arr, tmp_name)
        outs.append(tmp_name)
    return outs
...
def main(config):
    ...
    for lab_path in labs:
        ...
        outs=split_lab(img,config.tmp_path,f"{get_file_name(lab_path)}_lab",map_dic={0:0, 2221:1,1220:1,200:1, 500:1, 600:0})
        GT_list.extend(outs)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--tmp_path', type=str, default='./dataset/tmp/')
