...
from network import U_Net,R2U_Net,AttU_Net,R2AttU_Net,FCN
import csv
...
class Experiment(object):
	...
	def build_model(self):
		...
		if self.model_type =='U_Net':
			...
			self.unet = FCN(img_ch=self.config.img_ch,output_ch=self.config.output_ch)
			pass
	...
	def compute_accuracy(self,SR,GT):
		...
		GT_flat = GT.view(-1)
		if torch.cuda.is_available():
			acc = GT_flat.data.gpu()==(SR_flat.data.gpu()>0.5)
		else:
			...
			for epoch in range(self.num_epochs):
				...
				for i, (images, GT) in enumerate(self.train_loader):
					...
					for t in range(images.shape[0]):
						...
						self.write.add_image(f"SR", output, global_step=epoch*len(self.train_loader)+i)
